...
const DiscountClient = require('../models/discountClientZakaji');
const { setSingleOutXMLZakaji, cancelSingleOutXMLZakaji, setSingleOutXMLZakajiLogic } = require('../module/singleOutXMLZakaji');
const randomstring = require('randomstring');
const EmploymentZakaji = require('../models/employmentZakaji');
const { pubsub } = require('./index');
const { withFilter } = require('graphql-subscriptions');
const RELOAD_ORDER = 'RELOAD_ORDER';
const HistoryOrderZakaji = require('../models/historyOrderZakaji');
const { checkFloat } = require('../module/const');
const maxDates = 31
const { checkAdss } = require('../graphql/adsZakaji');
const SpecialPriceClientZakaji = require('../models/specialPriceClientZakaji');
...
const resolvers = {
    invoicesTrashSimpleStatistic: async(parent, {search}, {user}) => {
        ...
        let invoices = [];
        if(user.role==='admin') {
            invoices =  await InvoiceZakaji.find(
                {
                    del: 'deleted',
                    ...(search.length>0?{
                            $or: [
    ...
    invoices: async(parent, {search, sort, filter, date, skip, organization, city}, {user}) =>  {
        if(['суперорганизация', 'организация', 'client', 'admin', 'менеджер', 'агент', 'экспедитор', 'суперагент', 'суперэкспедитор'].includes(user.role)) {
            ...
            let invoices = await InvoiceZakaji.aggregate(
                [
                    {
                        $match: {
                            del: {$ne: 'deleted'},
                    ...
                    },
                    {
                        $lookup:
                            {
                                from: EmploymentZakaji.collection.collectionName,
                                let: {agent: '$agent'},
                                pipeline: [
                    ...
                    },
                    {
                        $lookup:
                            {
                                from: AdsZakaji.collection.collectionName,
                                let: {adss: '$adss'},
                                pipeline: [
    ...
    invoicesTrash: async(parent, {search, skip}, {user}) => {
        ...
        if(user.role==='admin') {
            return await InvoiceZakaji.aggregate(
                [
                    {
                        $match: {
                            del: 'deleted',
    ...
    orderHistorys: async(parent, {invoice}, {user}) => {
        if(['admin', 'менеджер', 'суперорганизация', 'организация'].includes(user.role)){
            let historyOrders =  await HistoryOrderZakaji.find({invoice: invoice}).sort('-createdAt').lean()
    ...
    isOrderToday: async(parent, {organization}, {user}) => {
        if('client'===user.role){
            ...
            dateEnd.setDate(dateEnd.getDate() + 1)
            let objectInvoice = await InvoiceZakaji.findOne({
                organization: organization,
                client: user.client,
                $and: [{createdAt: {$gte: dateStart}}, {createdAt: {$lt:dateEnd}}],
                del: {$ne: 'deleted'},
    ...
    invoice: async(parent, {_id}, {user}) => {
        if(['агент', 'менеджер', 'суперорганизация', 'организация', 'экспедитор', 'суперагент', 'admin', 'суперэкспедитор', 'client'].includes(user.role)) {
            return await InvoiceZakaji.findOne({
                _id: _id,
                ...user.client ? {client: user.client} : {},
                ...user.organization ?
                    {
    ...
    invoicesFromDistrict: async(parent, {organization, district, date}, {user}) =>  {
        if(['admin', 'агент', 'менеджер','суперорганизация', 'организация'].includes(user.role)) {
            ...
            let _clients = await DistrictZakaji.findOne({
                _id: district
            }).distinct('client').lean();
            if (['агент', 'менеджер'].includes(user.role)) {
                _clients = await DistrictZakaji
                    .find({$or: [{manager: user.employment}, {agent: user.employment}]})
                    .distinct('client').lean()
            }
            return await InvoiceZakaji.aggregate(
...
const resolversMutation = {
    acceptOrders: async(parent, ctx, {user}) => {
        if(user.role==='admin'){
            ...
            for(let i = 0; i<orders.length;i++) {
                ...
                let objectHistoryOrder = new HistoryOrderZakaji({
                ...
                await HistoryOrderZakaji.create(objectHistoryOrder);
                await orders[i].save()
    ...
    addOrders: async(parent, {priority, dateDelivery, info, paymentMethod, organization, client, inv, unite}, {user}) => {
        ...
        if(user.client)
            client = user.client
        client = await ClientZakaji.findOne({_id: client}).select('address id city').lean()
        let subbrand = await SubBrandZakaji.findOne({_id: organization}).select('organization').lean()
        if(subbrand)
            organization = subbrand.organization
        let baskets = await BasketZakaji.find(
            user.client?
                {client: user.client}:
        ...
        if(baskets.length>0){
            ...
            let dateEnd = new Date(dateStart)
            dateEnd.setDate(dateEnd.getDate() + 1)
            let superDistrict = await DistrictZakaji.findOne({
                organization: null,
                client: client._id
            }).select('agent').lean()
            ...
            if(distributers.length>0){
                for(let i=0; i<distributers.length; i++){
                    let findDistrict = await DistrictZakaji.findOne({
            ...
            let objectInvoice;
            if(unite&&!inv)
                objectInvoice = await InvoiceZakaji.findOne({
                    organization: organization,
                    client: client._id,
            ...
            if(!objectInvoice){
                ...
                for(let ii=0; ii<baskets.length;ii++){
                    let price = await SpecialPriceClientZakaji.findOne({
                    ...
                    let objectOrder = new OrderZakaji({
                        item: baskets[ii].item._id,
                        client: client._id,
                    ...
                    objectOrder = await OrderZakaji.create(objectOrder);
                    orders.push(objectOrder);
                }
                let number = randomstring.generate({length: 12, charset: 'numeric'});
                while (await InvoiceZakaji.findOne({number: number}).select('_id').lean())
                ...
                objectInvoice = new InvoiceZakaji({
                    guid,
                    city: client.city,
                    priority: priority,
                    discount: discount,
                ...
                if(inv)
                    objectInvoice.inv = 1
                objectInvoice = await InvoiceZakaji.create(objectInvoice);
            }
            else {
                for(let ii=0; ii<baskets.length;ii++){
                    let price
                    let objectOrder = await OrderZakaji.findOne({
                        item: baskets[ii].item._id,
                    ...
                    if(objectOrder){
                        ...
                        price = await SpecialPriceClientZakaji.findOne({
                            item: baskets[ii].item._id,
                            client: client._id
                        }).select('price').lean()
                        price = price?price.price:baskets[ii].item.price
                ...
                await OrderZakaji.updateMany({_id: {$in: objectInvoice.orders}}, {status: 'обработка', returned: 0})
                ...
                if(user.role==='admin'){
                    ...
                    let employment = await EmploymentZakaji.findOne({user: user._id}).lean()
            ...
            let newInvoice = await InvoiceZakaji.findOne({_id: objectInvoice._id})
            ...
            await BasketZakaji.deleteMany({_id: {$in: baskets.map(element=>element._id)}})
        }
        return {data: 'OK'};
    },
    deleteOrders: async(parent, {_id}, {user}) => {
        if(user.role==='admin'){
            let objects = await InvoiceZakaji.find({_id: {$in: _id}})
    ...
    setInvoicesLogic: async(parent, {track, forwarder, invoices}, {user}) => {
        await setSingleOutXMLZakajiLogic(invoices, forwarder, track)

        let resInvoices = await InvoiceZakaji.find({_id: {$in: invoices}})
            .select(' _id agent createdAt updatedAt allTonnage allSize client allPrice consignmentPrice returnedPrice info address paymentMethod discount adss editor number confirmationForwarder confirmationClient cancelClient district track forwarder sale provider organization cancelForwarder paymentConsignation taken sync city dateDelivery')
            .populate({path: 'client', select: '_id name email phone user', populate: [{path: 'user', select: '_id'}]})
            .populate({path: 'agent', select: '_id name'})
            .populate({path: 'sale', select: '_id name'})
    ...
    setOrder: async(parent, {orders, invoice}, {user}) => {
        let object = await InvoiceZakaji.findOne({_id: invoice})
            .populate({
                path: 'client'
            })
        let editor;
        ...
        let resInvoice = await InvoiceZakaji.findOne({_id: invoice})
            .populate({
                path: 'orders',
                populate: {
                    path: 'item'
        ...
        if(user.role==='admin'){
            ...
            let employment = await EmploymentZakaji.findOne({user: user._id}).select('name').lean()
            editor = `${user.role} ${employment.name}`
        }
        ...
        if((resInvoice.guid||resInvoice.dateDelivery>date)&&resInvoice.organization.pass&&resInvoice.organization.pass.length){
            if(resInvoice.orders[0].status==='принят') {
                resInvoice.sync = await setSingleOutXMLZakaji(resInvoice)
            }
            else if(resInvoice.orders[0].status==='отмена') {
                resInvoice.sync = await cancelSingleOutXMLZakaji(resInvoice)
            }
        }
    ...
    setInvoice: async(parent, {adss, taken, invoice, confirmationClient, confirmationForwarder, cancelClient, cancelForwarder, paymentConsignation}, {user}) => {
        let object = await InvoiceZakaji.findOne({_id: invoice}).populate('client').populate('order')
        ...
        if(taken!=undefined&&(admin||employment)){
            object.taken = taken
            if(taken) {
                await OrderZakaji.updateMany({_id: {$in: object.orders}}, {status: 'принят'})
            }
            else {
                await OrderZakaji.updateMany({_id: {$in: object.orders}}, {status: 'обработка', returned: 0})
        ...
        if(object.taken&&object.confirmationForwarder&&object.confirmationClient){
            await OrderZakaji.updateMany({_id: {$in: object.orders}}, {status: 'выполнен'})
        }

        if(object.taken&&(object.confirmationForwarder||object.confirmationClient)){
            let route = await RouteZakaji.findOne({invoices: invoice}).populate({
                path: 'invoices',
                populate : {
                    path : 'orders',
                }
        ...
        if(cancelClient!=undefined&&(cancelClient||object.cancelClient!=undefined)&&!object.cancelForwarder&&(admin||client)){
            if(cancelClient){
                ...
                await OrderZakaji.updateMany({_id: {$in: object.orders}}, {status: 'отмена'})
            }
                ...
                if (differenceMinutes < 10||user.role==='admin') {
                    ...
                    await OrderZakaji.updateMany({_id: {$in: object.orders}}, {status: 'обработка'})
