...
const { setSingleOutXMLReturnedZakaji, cancelSingleOutXMLReturnedZakaji, setSingleOutXMLReturnedZakajiLogic } = require('../module/singleOutXMLZakaji');
const { pubsub } = require('./index');
const { checkFloat } = require('../module/const');
const { withFilter } = require('graphql-subscriptions');
const RELOAD_RETURNED = 'RELOAD_RETURNED';
const HistoryReturnedZakaji = require('../models/historyReturnedZakaji');
...
const resolvers = {
    returnedsTrashSimpleStatistic: async(parent, {search}, {user}) => {
        ...
        if(search.length>0){
            ...
            _clients = await ClientZakaji.find({
                $or: [
                    {name: {'$regex': search, '$options': 'i'}},
        ...
        if(user.role==='admin') {
            returneds =  await ReturnedZakaji.find(
                {
                    del: 'deleted',
                    ...(search.length>0?{
                            $or: [
    ...
    returnedsTrash: async(parent, {search, skip}, {user}) => {
        ...
        if(user.role==='admin') {
            let returneds =  await ReturnedZakaji.aggregate(
                [
                    {
                        $match:{
                            del: 'deleted',
    ...
    returnedsSimpleStatistic: async(parent, {search, date, city}, {user}) => {
        if(['суперорганизация', 'организация', 'агент', 'менеджер', 'admin', 'суперагент'].includes(user.role)) {
            ...
            let returneds = await ReturnedZakaji.find({
                del: {$ne: 'deleted'},
    ...
    returnedsFromDistrict: async(parent, {organization, district, date}, {user}) =>  {
        if(['суперорганизация', 'организация', 'агент', 'менеджер', 'admin'].includes(user.role)) {
            ...
            if (['агент', 'менеджер'].includes(user.role)) {
                ...
                clients = await DistrictZakaji.findOne({
                    _id: district,
                }).distinct('client').lean();
            }
            return await ReturnedZakaji.aggregate(
                [
                    {
                        $match: {
                            del: {$ne: 'deleted'},
    ...
    returnedHistorys: async(parent, {returned}, {user}) => {
        if(['admin', 'менеджер', 'суперорганизация', 'организация'].includes(user.role)){
            let historyReturneds =  await HistoryReturnedZakaji.find({returned: returned}).lean()
    ...
    returneds: async(parent, {search, sort, date, skip, city}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'суперагент', 'агент'].includes(user.role)) {
            ...
            if(search.length>0){
                ...
                _clients = await ClientZakaji.find({$or: [
                    {name: {'$regex': search, '$options': 'i'}},
                    {address: {$elemMatch: {$elemMatch: {'$regex': search, '$options': 'i'}}}}
                ]
                }).distinct('_id').lean()
...
const resolversMutation = {
    addReturned: async(parent, {info, address, organization, client, items, inv}, {user}) =>     {
        ...
        let city = (await ClientZakaji.findById(client).select('city').lean()).city
        ...
        let objectReturned = await ReturnedZakaji.findOne({
            organization: organization,
            client: client,
            $and: [{createdAt: {$gte: dateStart}}, {createdAt: {$lt:dateEnd}}],
            del: {$ne: 'deleted'},
        ...
        if(!objectReturned){
            ...
            while (await ReturnedZakaji.findOne({number: number}).select('_id').lean())
            ...
            objectReturned = new ReturnedZakaji({
                guid,
                items: items,
                client: client,
                allPrice: allPrice,
            ...
            objectReturned = await ReturnedZakaji.create(objectReturned);
        }
        else{
            for(let i = 0; i< items.length; i++){
                let have = false
    ...
    deleteReturneds: async(parent, {_id}, {user}) => {
        if(user.role==='admin'){
            let objects = await ReturnedZakaji.find({_id: {$in: _id}})
    ...
    restoreReturneds: async(parent, {_id}, {user}) => {
        if(user.role==='admin'){
            await ReturnedZakaji.updateMany({_id: {$in: _id}}, {del: null})
        }
        return {data: 'OK'};
    },
    setReturnedLogic: async(parent, {track, forwarder, returneds}) => {
        await setSingleOutXMLReturnedZakajiLogic(returneds, forwarder, track)
        return {data: 'OK'};
    },
    setReturned: async(parent, {items, returned, confirmationForwarder, cancelForwarder}, {user}) => {
        let object = await ReturnedZakaji.findOne({_id: returned})
            .populate({
                path: 'client'
            })
        ...
        if(object.organization.pass&&object.organization.pass.length){
            if(object.confirmationForwarder) {
                await setSingleOutXMLReturnedZakaji(object)
            }
            else if(object.cancelForwarder) {
                await cancelSingleOutXMLReturnedZakaji(object)
            }
        }

        let objectHistoryReturned = new HistoryReturnedZakaji({
            returned: returned,
            editor: editor,
        });
        await HistoryReturnedZakaji.create(objectHistoryReturned);
        pubsub.publish(RELOAD_RETURNED, { reloadReturned: {
            who: user.role==='admin'?null:user._id,
            client: object.client._id,
