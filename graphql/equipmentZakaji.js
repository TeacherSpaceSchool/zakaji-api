...
const resolvers = {
    repairEquipments: async(parent, {organization, search, filter}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент', 'ремонтник'].includes(user.role)) {
            ...
            let _agents = [];
            if(search.length>0) {
                _clients = await ClientZakaji.find({del: {$ne: 'deleted'}, $or: [{name: {'$regex': search, '$options': 'i'}},{info: {'$regex': search, '$options': 'i'}}, {address: {$elemMatch: {$elemMatch: {'$regex': search, '$options': 'i'}}}}]})
                    .distinct('_id').lean()
                _agents = await EmploymentZakaji.find({
                    name: {'$regex': search, '$options': 'i'}
                }).distinct('_id').lean()
            }
            let repairEquipments = await RepairEquipmentZakaji.find({
    ...
    repairEquipment: async(parent, {_id}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент', 'ремонтник'].includes(user.role)) {
            return await RepairEquipmentZakaji.findOne({
                _id: _id,
...
const resolversMutation = {
    addRepairEquipment: async(parent, {equipment, client, defect, organization}, {user}) => {
        if(['агент', 'admin', 'суперагент', 'суперорганизация', 'организация'].includes(user.role)){
            ...
            while (await RepairEquipmentZakaji.findOne({number: number}).select('_id').lean())
                number = randomstring.generate({length: 12, charset: 'numeric'});
            let _object = new RepairEquipmentZakaji({
                number: number,
                status: 'обработка',
                equipment: equipment,
                client,
            ...
            await RepairEquipmentZakaji.create(_object)
        }
        return {data: 'OK'};
    },
    setRepairEquipment: async(parent, {_id, accept, done, cancel, defect, repair, equipment, client}, {user}) => {
        if(['агент', 'admin', 'суперагент', 'суперорганизация', 'организация', 'ремонтник'].includes(user.role)) {
            let object = await RepairEquipmentZakaji.findById(_id)
    ...
    deleteRepairEquipment: async(parent, { _id }, {user}) => {
        if(['агент', 'admin', 'суперагент', 'суперорганизация', 'организация'].includes(user.role)){
            await RepairEquipmentZakaji.deleteMany({_id: {$in: _id}, ...user.organization?{organization: user.organization}:{}})
        }
        return {data: 'OK'}
    },
};
