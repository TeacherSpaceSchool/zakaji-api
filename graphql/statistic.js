...
const query = `
    ...
    statisticZakajiOrder(company: ID, filter: String, dateStart: Date, dateType: String, city: String): Statistic
    statisticZakajiAgents(company: ID, dateStart: Date, dateType: String, filter: String, city: String): Statistic
    statisticZakajiAgent(agent: ID!, dateStart: Date, dateType: String): Statistic
...
const resolvers = {
    checkIntegrateClient: async(parent, { organization, type, document }, {user}) => {
        if(user.role==='admin'){
            if(type!=='отличая от 1С') {
                let statistic = [];
                let sortStatistic = {};
                let data = await Integrate1CZakaji.find(
                    {
                ...
                for (let i = 0; i < rows.length; i++) {
                    let integrate1CZakaji = await Integrate1CZakaji.findOne({
                        ...
                        .lean()
                    if(integrate1CZakaji&&integrate1CZakaji.client.address[0]&&integrate1CZakaji.client.address[0][2]) {
                        let market = rows[i][1].toString().toLowerCase()
                        while (market.includes(' '))
                            market = market.replace(' ', '')
                        while (market.includes('-'))
                            market = market.replace('-', '')
                        let market1 = integrate1CZakaji.client.address[0][2].toLowerCase()
                        ...
                        if (problem) {
                            statistic.push({
                                _id: i, data: [
                                    integrate1CZakaji.guid,
                                    //integrate1CZakaji.client.address[0][2],
                                    `${integrate1CZakaji.client.address && integrate1CZakaji.client.address[0] ? `${integrate1CZakaji.client.address[0][2] ? `${integrate1CZakaji.client.address[0][2]}, ` : ''}${integrate1CZakaji.client.address[0][0]}` : ''}`,
                                    rows[i][1]
                                ]
                            })
                        }
    ...
    checkAgentRoute: async(parent, { agentRoute }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            let problem = []
            let data = await AgentRouteZakaji.findOne(
                {
                    _id: agentRoute
                }
            )
    ...
    checkOrder: async(parent, { company, today, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            let repeat = 0
            let noSync = 0
                let data = await InvoiceZakaji.find(
                {
                    $and: [
    ...
    statisticOrderChart: async(parent, { company, dateStart, dateType, type, online, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            let clients = {}
            if(online){
                excludedAgents = await UserZakaji.find({$or: [{role: 'агент'}, {role: 'менеджер'}, {role: 'организация'}, {role: 'суперорганизация'}]}).distinct('_id').lean()
                excludedAgents = await EmploymentZakaji.find({user: {$in: excludedAgents}}).distinct('_id').lean()
            }
            if(dateStart){
                let organizations
                if(!company){
                    organizations = await OrganizationZakaji.find({...city?{cities: city}:{}})
                        .select('_id name')
                        .lean()
                }
                else {
                    organizations = await OrganizationZakaji.find({_id: company})
                        .select('_id name')
                        .lean()
                }
    ...
    statisticClientActivity: async(parent, { online, organization, city } , {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            if(online){
                excludedAgents = await UserZakaji.find({$or: [{role: 'агент'}, {role: 'менеджер'}, {role: 'организация'}, {role: 'суперорганизация'}]}).distinct('_id').lean()
                excludedAgents = await EmploymentZakaji.find({
            ...
            let orderClient = []
            let orderByClient = {}
            let data =  await InvoiceZakaji.aggregate(
                [
                    {
                        $match:{
                            ...(city?{city: city}:{}),
    ...
    statisticStorageSize: async(parent, ctx, {user}) => {
        if(['admin'].includes(user.role)){
            ...
            let data = []
            let collections = [
                {name: 'Маршрут агента', collection: '../models/agentRouteZakaji'},
                {name: 'Акции', collection: '../models/adsZakaji'},
                {name: 'История посещения агентов',collection: '../models/agentHistoryGeoZakaji'},
                {name: 'Маршрут агента', collection: '../models/agentRouteZakaji'},
                {name: 'Транспорт', collection: '../models/autoZakaji'},
                {name: 'Корзина', collection: '../models/basketZakaji'},
                {name: 'Блог', collection: '../models/blogZakaji'},
                {name: 'Категории', collection: '../models/categoryZakaji'},
                {name: 'Клиенты', collection: '../models/clientZakaji'},
                {name: 'Контакты', collection: '../models/contactZakaji'},
                {name: 'Дни доставки', collection: '../models/deliveryDateZakaji'},
                {name: 'Скидка', collection: '../models/discountClientZakaji'},
                {name: 'Дистрибьютор', collection: '../models/distributerZakaji'},
                {name: 'Районы', collection: '../models/districtZakaji'},
                {name: 'Сотрудники', collection: '../models/employmentZakaji'},
                {name: 'Ошибка', collection: '../models/errorZakaji'},
                {name: 'FAQ', collection: '../models/faqZakaji'},
                {name: 'Формы', collection: '../models/form/formZakaji'},
                {name: 'Шаблоны форм', collection: '../models/form/templateFormZakaji'},
                {name: 'История заказов', collection: '../models/historyOrderZakaji'},
                {name: 'История возратов', collection: '../models/historyReturnedZakaji'},
                {name: 'Интеграция 1С', collection: '../models/integrate1CZakaji'},
                {name: 'Накладные', collection: '../models/invoiceZakaji'},
                {name: 'Товары', collection: '../models/itemZakaji'},
                {name: 'Лотерея', collection: '../models/lotteryZakaji'},
                {name: 'Мерчендайзинг', collection: '../models/merchandisingZakaji'},
                {name: 'Уведомления', collection: '../models/notificationStatisticZakaji'},
                {name: 'Заказы', collection: '../models/orderZakaji'},
                {name: 'Организации', collection: '../models/organizationZakaji'},
                {name: 'Принятая интеграция', collection: '../models/receivedDataZakaji'},
                {name: 'Ремонт инвентаря', collection: '../models/repairEquipmentZakaji'},
                {name: 'Возвраты', collection: '../models/returnedZakaji'},
                {name: 'Отзывы', collection: '../models/reviewZakaji'},
                {name: 'Маршруты', collection: '../models/routeZakaji'},
                {name: 'Акционая интеграция', collection: '../models/singleOutXMLAdsZakaji'},
                {name: 'Выгрузка заказов', collection: '../models/singleOutXMLZakaji'},
                {name: 'Выгрузка вощвратов', collection: '../models/singleOutXMLReturnedZakaji'},
                {name: 'Подкатегории', collection: '../models/subCategoryZakaji'},
                {name: 'Подписчики', collection: '../models/subscriberZakaji'},
                {name: 'Пользователи', collection: '../models/userZakaji'}
            ]
    ...
    statisticClientCity: async(parent, ctx, {user}) => {
        if(['admin'].includes(user.role)){
            ...
            let count
            let data = []
            let clients = await UserZakaji.find({role: 'client', status: 'active'}).distinct('_id').lean()
            for(let i=0; i<cities.length; i++){
                count = await ClientZakaji.countDocuments({
                    user: {$in: clients},
                    del: {$ne: 'deleted'},
                    city: cities[i]
                })
    ...
    statisticOrganizationActivity: async(parent, { online, organization, city } , {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            if(!organization){
                ...
                data = await DistrictZakaji.find({organization: organization})
    ...
    statisticAdss: async(parent, { company, dateStart, dateType, online, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            let adss = await AdsZakaji.find({ ...(company==='all'?{}:{ organization: company }),}).distinct('_id').lean()
            let data = await InvoiceZakaji.find(
                {
                    $and: [
                        dateStart?{createdAt: {$gte: dateStart}}:{},
    ...
    statisticDistributer: async(parent, { distributer, organization, dateStart, dateType, type, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            let statistic = {}, data = []
            if(distributer){
                let findDistributer = await DistributerZakaji.findOne(
                    distributer!=='super'?
                ...
                if(findDistributer){
                    if(type==='all') {
                        let clients = await DistrictZakaji
                        ...
                        let districts = await DistrictZakaji
                            .find({organization: distributer!=='super'?distributer:null})
                        let organizations = []
                        if(organization){
                            for (let i = 0; i < findDistributer.sales.length; i++) {
                        ...
                        data = await InvoiceZakaji.find(
                            ...
                            .lean()
                        for (let i1 = 0; i1 < data.length; i1++) {
                            let name = data[i1].agent&&data[i1].agent.organization&&data[i1].agent.organization.toString()===distributer.toString()?data[i1].agent.name:'ZAKAJI.KG'
                            let id = data[i1].agent&&data[i1].agent.organization&&data[i1].agent.organization.toString()===distributer.toString()?data[i1].agent._id:'ZAKAJI.KG'
                            if (!statistic[id]) statistic[id] = {
                                profit: 0,
                                returned: 0,
                                cancel: [],
    ...
    statisticOrder: async(parent, { company, dateStart, dateType, online, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            if(!company) {
                ...
                let data = await DistrictZakaji.find({
    ...
    statisticSubBrand: async(parent, { company, dateStart, dateType, online, city }, {user}) => {
        if('admin'===user.role){
            ...
            let items = await ItemZakaji.find({
                ...company?{organization: company}:{},
                subBrand: {$ne: null}
            })
    ...
    statisticZakajiOrder: async(parent, { company, dateStart, dateType, filter, city }, {user}) => {
        if(['admin'].includes(user.role)){
            let dateEnd
            let statistic = {}, data = []
            let priceAll = 0
            ...
            let organizations
            let agents = await EmploymentZakaji.find({organization: null})
                .select('_id')
                .populate({
                    path: 'user',
                    select: 'role'
            ...
            if(!company){
                organizations = await OrganizationZakaji.find({superagent: true, ...city?{cities: city}:{}}).distinct('_id')
            }
            else {
                organizations = await OrganizationZakaji.find({_id: company, superagent: true, ...city?{cities: city}:{}}).distinct('_id')
            }
            data = await InvoiceZakaji.find(
                {
                    $and: [
        ...
        if(['admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)){
            ...
            if(!agent) {
                let districts = await DistrictZakaji.find({
                    ...
                    .lean()
                let data = await MerchandisingZakaji.find({
                    $and: [
                        dateStart ? {createdAt: {$gte: dateStart}} : {},
                        dateEnd ? {createdAt: {$lt: dateEnd}} : {}
                    ],
    ...
    statisticDevice: async(parent, { filter }, {user}) => {
        if(['admin'].includes(user.role)){
            let statistic = {}
            let data = await ClientZakaji.find({device: {$ne: null}})
                .select('device')
                .lean()
            let device
            for(let i=0; i<data.length; i++) {
    ...
    statisticReturned: async(parent, { company, dateStart, dateType, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            if(!company) {
                data = await ReturnedZakaji.find(
                    {
                        $and: [
                            dateStart ? {createdAt: {$gte: dateStart}} : {},
                            dateEnd ? {createdAt: {$lt: dateEnd}} : {}
    ...
    statisticAgents: async(parent, { company, dateStart, dateType, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            if(!company) {
                ...
                for(let i=0; i<data.length; i++) {
                    let agent = !data[i].agent||['суперменеджер', 'суперагент', 'суперэкспедитор'].includes(data[i].agent.user.role)?{_id: 'ZAKAJI.KG', name: 'ZAKAJI.KG'}:{name: data[i].agent.name, _id: data[i].agent._id.toString()}
    ...
    statisticZakajiAgents: async(parent, { company, dateStart, dateType, filter, city }, {user}) => {
        if('admin'===user.role){
            let dateEnd
            if(dateStart){
                dateStart= new Date(dateStart)
            ...
            if(!company) {
                for(let i=0; i<data.length; i++) {
                    ...
                    let name
                    if(filter==='агент'){
                        name = data[i].agent?data[i].agent.name:'ZAKAJI.KG'
                        id = data[i].agent?data[i].agent._id:'ZAKAJI.KG'
                    }
                    else{
                ...
                for(let i=0; i<data.length; i++) {
                    let name = data[i].agent?data[i].agent.name:'ZAKAJI.KG'
                    let id = data[i].agent?data[i].agent._id:'ZAKAJI.KG'
                    if (!statistic[id])
                        statistic[id] = {
                            price: 0,
                            returned: 0,
    ...
    statisticZakajiAgent: async(parent, { agent, dateStart, dateType }, {user}) => {
        if('admin'===user.role){
            let dateEnd
            if(dateStart){
                dateStart= new Date(dateStart)
    ...
    statisticAgentsWorkTime: async(parent, { organization, date }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            dateEnd.setDate(dateEnd.getDate() + 1)
            let data = []
            let agents = await UserZakaji.find({
                ...(organization!=='super'?
                ...
                .distinct('_id')
                .lean()
            agents = await EmploymentZakaji.find({
    ...
    activeOrganization: async(parent, {city}, {user}) => {
        if('admin'===user.role){
            let data = await OrganizationZakaji.find(
                {
                    ...city?{cities: city}:{},
                }
            )
    ...
    statisticClientGeo: async(parent, { search, organization, item, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            organization = user.organization?user.organization:organization
            let clients = await UserZakaji.find({role: 'client', status: 'active'}).distinct('_id').lean()
            clients = await ClientZakaji.find({
                user: {$in: clients},
                del: {$ne: 'deleted'},
                ...city?{city: city}:{},
            ...
            if(item){
                invoices =  await OrderZakaji.aggregate(
                    [
                        {
                            $match:{
                                createdAt: {$gte: dateStart},
                ...
                invoices =  await InvoiceZakaji.aggregate(
                    [
                        {
                            $match:{
                                createdAt: {$gte: dateStart},
    ...
    unloadingInvoices: async(parent, { organization, dateStart, forwarder, all }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            if(all){
                let distributers = await DistributerZakaji.findOne({
                    distributer: organization==='super'?null:organization
                })
                let clients = await DistrictZakaji.find({
            ...
            if(organization!=='super') {
                ...
                organization = await ContactZakaji.findOne()
            }
            let worksheet;
            let auto
            let items = {}
            ...
            for(let i = 0; i<data.length;i++){
                ...
                if(forwarder){
                    let district = await DistrictZakaji.findOne({client: data[i].client._id, organization: forwarder!=='super'?forwarder:null}).populate('ecspeditor').lean()
                ...
                if(data[i].forwarder){
                    ...
                    auto = await AutoZakaji.findOne({employment: data[i].forwarder._id})
                    row+=1;
                    worksheet.getCell(`A${row}`).font = {bold: true};
    ...
    unloadingClients: async(parent, { organization, city }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            ...
            let cities
            if(organization!=='super')
                cities = (await OrganizationZakaji.findById(organization).select('cities').lean()).cities
            let workbook = new ExcelJS.Workbook();
            let data = await ClientZakaji.find(
                {
            ...
            for(let i = 0; i<data.length;i++){
                let GUID = ''
                let findGUID = await Integrate1CZakaji.findOne({
    ...
    unloadingEmployments: async(parent, { organization }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            organization = user.organization?user.organization:organization
            let workbook = new ExcelJS.Workbook();
            let data = await EmploymentZakaji.find(
                {
                    ...(organization==='super'?{organization: null}:{organization: organization}),
                    ...{del: {$ne: 'deleted'}}
                }
    ...
    unloadingAgentRoutes: async(parent, { organization }, {user}) => {
        if(['admin', 'суперорганизация'].includes(user.role)){
            organization = user.organization?user.organization:organization
            let workbook = new ExcelJS.Workbook();
            let data = await AgentRouteZakaji.find(
                {
                    ...(organization==='super'?{organization: null}:{organization: organization})
                }
            ).lean()
...
const resolversMutation = {
        ...
        if (user.role === 'admin') {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                if(!integrate1CZakaji) {
                    ...
                    item = await ItemZakaji.findOne({_id: integrate1CZakaji.item, organization})
                    item.name = rows[i][1]
    ...
    uploadingClients: async(parent, { document, organization, city }, {user}) => {
        if (user.role === 'admin') {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                if(!integrate1CZakaji) {
                    let client = new UserZakaji({
                        login: randomstring.generate(20),
                        role: 'client',
                        status: 'deactive',
                        password: '12345678',
                    });
                    client = await UserZakaji.create(client);
                    client = new ClientZakaji({
                        name: rows[i][1],
                        phone: [''],
                        city,
                    ...
                    client = await ClientZakaji.create(client);
    ...
    uploadingAgentRoute: async(parent, { document, agentRoute }, {user}) => {
        if (user.role === 'admin') {
            ...
            let xlsxpath = path.join(app.dirname, 'public', filename);
            let rows = await readXlsxFile(xlsxpath)
            agentRoute = await AgentRouteZakaji.findOne({_id: agentRoute})
            ...
            if(agentRoute){
                ...
                for (let i = 0; i < rows.length; i++) {
                    if(rows[i][0]&&rows[i][0].length>0){
                        ...
                        if (integrate1CZakaji && integrate1CZakaji.client) {
                            agentRoute.clients[0].push(integrate1CZakaji.client)
                        }
                    }
                    if(rows[i][1]&&rows[i][1].length>0){
                        ...
                        if (integrate1CZakaji && integrate1CZakaji.client) {
                            agentRoute.clients[1].push(integrate1CZakaji.client)
                        }
                    }
                    if(rows[i][2]&&rows[i][2].length>0){
                        ...
                        if (integrate1CZakaji && integrate1CZakaji.client) {
                            agentRoute.clients[2].push(integrate1CZakaji.client)
                        }
                    }
                    if(rows[i][3]&&rows[i][3].length>0){
                        ...
                        if (integrate1CZakaji && integrate1CZakaji.client) {
                            agentRoute.clients[3].push(integrate1CZakaji.client)
                        }
                    }
                    if(rows[i][4]&&rows[i][4].length>0){
                        ...
                        if (integrate1CZakaji && integrate1CZakaji.client) {
                            agentRoute.clients[4].push(integrate1CZakaji.client)
                        }
                    }
                    if(rows[i][5]&&rows[i][5].length>0){
                        ...
                        if (integrate1CZakaji && integrate1CZakaji.client) {
                            agentRoute.clients[5].push(integrate1CZakaji.client)
                        }
                    }
                    if(rows[i][6]&&rows[i][6].length>0){
                        ...
                        if (integrate1CZakaji && integrate1CZakaji.client) {
                            agentRoute.clients[6].push(integrate1CZakaji.client)
                        }
                    }
                }
        ...
        if (user.role === 'admin') {
            ...
            for (let i = 0; i < rows.length; i++) {
                if(!findEmployments[rows[i][0]]||!districts[findEmployments[rows[i][0]]]){
                    ...
                    if (integrate1CZakaji && integrate1CZakaji.agent) {
                        findEmployments[rows[i][0]] = integrate1CZakaji.agent
            ...
            for (let i = 0; i < keys1.length; i++) {
                district = await DistrictZakaji.findOne({
                    ...(organization==='super'?{organization: null}:{organization: organization}),
                    agent: keys1[i]
                })
                if(district) {
                    ...
                    for (let i1 = 0; i1 < districts[keys1[i]].length; i1++) {
                        ...
                        if (integrate1CZakaji && integrate1CZakaji.client) {
                            district.client.push(integrate1CZakaji.client)
                        }
                    }
                    await district.save()
                }
