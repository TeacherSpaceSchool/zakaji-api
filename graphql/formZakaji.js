const TemplateFormZakaji = require('../models/form/templateFormZakaji');
const FormZakaji = require('../models/form/formZakaji');
...
const resolvers = {
    templateForms: async(parent, {search, organization, skip}, {user}) => {
        if(['client', 'агент', 'admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)){
            let templateForms
            if(user.role==='client')
                templateForms = await FormZakaji.find({client: user.client}).distinct('templateForm').lean()
            return await TemplateFormZakaji.find({
    ...
    analysisForms: async(parent, {templateForm}, {user}) => {
        if(['суперорганизация', 'организация', 'admin'].includes(user.role)) {
            let forms = await FormZakaji.find({...user.organization?{organization: user.organization}:{}, templateForm: templateForm})
    ...
    templateForm: async(parent, {_id}, {user}) => {
        if(['суперорганизация', 'организация', 'менеджер', 'агент', 'admin', 'client'].includes(user.role)){
            return await TemplateFormZakaji.findOne({
                _id: _id,
    ...
    forms: async(parent, {templateForm, search, skip}, {user}) => {
        if(['агент', 'суперорганизация', 'организация', 'менеджер', 'admin'].includes(user.role)){
            ...
            if(['агент', 'менеджер'].includes(user.role)){
                districtClients = await DistrictZakaji
                    .find({$or: [{manager: user.employment}, {agent: user.employment}]})
                    .distinct('client')
                if(user.onlyIntegrate){
                    districtClients = await Integrate1CZakaji
                        .find({client: {$in: districtClients}, organization: user.organization})
                        .distinct('client')
                        .lean()
                }
                templateForm = (await TemplateFormZakaji.findOne({_id: templateForm, organization: user.organization, editorEmployment: true}).select('_id').lean())._id
            }
            let _clients;
            if(search.length>0) {
    ...
    form: async(parent, {_id}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент'].includes(user.role)){
            return await FormZakaji.findOne({
                _id: _id,
                ...user.organization?{organization: user.organization}:{}
            })
...
const resolversMutation = {
    addTemplateForm: async(parent, {title, organization, editorEmployment, editorClient, edit, questions}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)) {
            let newTemplateForm = new TemplateFormZakaji({
                title: title,
            ...
            await TemplateFormZakaji.create(newTemplateForm);
        }
        return {data: 'OK'}
    },
    setTemplateForm: async(parent, {_id, title, editorEmployment, editorClient, edit, questions}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)) {
            let templateForm = await TemplateFormZakaji.findOne({_id: _id, ...user.organization?{organization: user.organization}:{}})
            if(templateForm){
    ...
    addForm: async(parent, {templateForm, client, questions}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент', 'client'].includes(user.role)) {
            templateForm = await TemplateFormZakaji.findOne({_id: templateForm, ...user.organization?{organization: user.organization}:{}}).select('organization _id').lean()
            if(user.client) client = user.client
            if(templateForm) {
                let newForm = new FormZakaji({
                ...
                await FormZakaji.create(newForm);
            }
        }
        return {data: 'OK'}
    },
    setForm: async(parent, {_id, questions}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент', 'client'].includes(user.role)) {
            let form = await FormZakaji.findOne({_id: _id, ...user.organization?{organization: user.organization}:{}})
            if(form) {
                let templateForm = await TemplateFormZakaji.findOne({_id: form.templateForm}).select('edit').lean()
    ...
    deleteTemplateForm: async(parent, {_id}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)) {
            await TemplateFormZakaji.deleteMany({_id: {$in: _id}})
            await FormZakaji.deleteMany({templateForm: {$in: _id}})
        }
        return {data: 'OK'}
    },
    deleteForm: async(parent, {_id}, {user}) => {
        if(['admin', 'суперорганизация', 'организация', 'менеджер', 'агент'].includes(user.role)) {
            await FormZakaji.deleteMany({_id: {$in: _id}})
        }
        return {data: 'OK'}
    },
