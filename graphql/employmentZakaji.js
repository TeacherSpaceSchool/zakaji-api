...
const resolvers = {
    employmentsTrash: async(parent, {search}, {user}) => {
        if(user.role==='admin') {
            return await EmploymentZakaji.find({
                del: 'deleted',
    ...
    ecspeditors: async(parent, {_id}, {user}) => {
        if (['суперорганизация', 'организация', 'менеджер', 'агент', 'admin'].includes(user.role)) {
            let users = await UserZakaji.find({
    ...
    employment: async(parent, {_id}, {user}) => {
        if(user.role&&user.role!=='client') {
            let result = await EmploymentZakaji.findOne({
                $or: [
                    {_id: _id},
                    {user: _id}
                ],
            ...
            if(result === null||!['admin', 'суперорганизация', 'организация'].includes(user.role))
                return await EmploymentZakaji.findOne({user: user._id})
                    .populate({
                        path: 'user',
                        select: '_id role status login'
                    })
...
const resolversMutation = {
    addEmployment: async(parent, {name, email, phone, login, password, role, organization}, {user}) => {
        if(user.role==='admin') {
            let newUser = new UserZakaji({
                login: login.trim(),
                role: role,
                status: 'active',
                password: password,
            });
            newUser = await UserZakaji.create(newUser);
            const client = new EmploymentZakaji({
                name: name,
                email: email,
                phone: phone,
                organization: organization,
            ...
            await EmploymentZakaji.create(client);
        }
        return {data: 'OK'}
    },
    setEmployment: async(parent, {_id, name, email, newPass, role, login, phone}, {user, res}) => {
        if(['admin', 'суперорганизация', 'организация'].includes(user.role)) {
            let object = await EmploymentZakaji.findOne({_id: _id, ...user.organization?{organization: user.organization}:{}})
            if(role==='суперорганизация'&&user.role!=='admin')
                role = 'организация'
            if (role || newPass || login) {
                let objectUser = await UserZakaji.findById(object.user)
    ...
    deleteEmployment: async(parent, { _id }, {user}) => {
        let objects = await EmploymentZakaji.find({_id: {$in: _id}})
        for(let i=0; i<objects.length; i++){
            if(user.role==='admin'){
                await EmploymentZakaji.update({_id: objects[i]._id}, {del: 'deleted'})
                await UserZakaji.update({_id: objects[i].user}, {status: 'deactive'})
                await Integrate1CZakaji.deleteOne({
                ...
                let district = await DistrictZakaji.find({
                    organization: objects[i].organization,
                    $or:
                        [
                            {manager: objects[i]._id},
    ...
    restoreEmployment: async(parent, { _id }, {user}) => {
        let objects = await EmploymentZakaji.find({_id: {$in: _id}}).lean()
        for(let i=0; i<objects.length; i++){
            if(user.role==='admin'){
                await EmploymentZakaji.update({_id: objects[i]._id}, {del: null})
                await UserZakaji.update({_id: objects[i].user}, {status: 'active'})
            }
        }
        return {data: 'OK'}
    },
    onoffEmployment: async(parent, { _id }, {user}) => {
        ...
        for(let i=0; i<objects.length; i++){
            if(user.role==='admin'||(['суперорганизация', 'организация'].includes(user.role)&&user.organization.toString()===objects[i].organization.toString())) {
                let object = await UserZakaji.findOne({_id: objects[i].user})
