...
const resolvers = {
    ...
    items: async(parent, {organization, subCategory, search, sort}, {user}) => {
        if(['admin', 'суперагент', 'экспедитор', 'суперорганизация', 'организация', 'менеджер', 'агент', 'client'].includes(user.role)){
            ...
            if(user.organization)
                organizations = [user.organization, ...(await DistributerZakaji.findOne({
    ...
    brands: async(parent, {organization, search, sort, city}, {user}) => {
        if(mongoose.Types.ObjectId.isValid(organization)) {
            let subBrand = await SubBrandZakaji.findOne({_id: organization}).select('organization _id').lean()
            if(subBrand){
    ...
    item: async(parent, {_id}) => {
        if(mongoose.Types.ObjectId.isValid(_id)) {
            return await ItemZakaji.findOne({
                _id: _id,
            })
                .populate({
                    path: 'subCategory',
...
const resolversMutation = {
    addItem: async(parent, {subBrand, categorys, city, unit, apiece, costPrice, priotiry, name, image, info, price, subCategory, organization, hit, latest, packaging, weight, size}, {user}) => {
        if(['admin', 'суперорганизация', 'организация'].includes(user.role)){
            ...
            await ItemZakaji.create(_object)
        }
        return {data: 'OK'};
    },
    setItem: async(parent, {subBrand, city, unit, categorys, apiece, costPrice, _id, priotiry, weight, size, name, image, info, price, subCategory, organization, packaging, hit, latest}, {user}) => {
         if(['admin', 'суперорганизация', 'организация'].includes(user.role)) {
            let object = await ItemZakaji.findOne({
                _id: _id,
    ...
    setItemsCostPrice: async(parent, { itemsCostPrice }, {user}) => {
        if(user.role==='admin') {
            ...
            for (let i = 0; i < itemsCostPrice.length; i++) {
                object = await ItemZakaji.findOne({_id: itemsCostPrice[i]._id})
    ...
    onoffItem: async(parent, { _id }, {user}) => {
        let objects = await ItemZakaji.find({_id: {$in: _id}})
        for(let i=0; i<objects.length; i++){
            if(user.role==='admin'|| (['суперорганизация', 'организация'].includes(user.role)&&user.organization.toString()===objects[i].organization.toString())){
                ...
                await BasketZakaji.deleteMany({item: {$in: objects[i]._id}})
            }
        }
        return {data: 'OK'}
    },
    deleteItem: async(parent, { _id }, {user}) => {
        if(['admin', 'суперорганизация', 'организация'].includes(user.role)) {
            await ItemZakaji.updateMany({_id: {$in: _id}}, {
                del: 'deleted',
                status: 'deactive'
            })
            let objects = await ItemZakaji.find({_id: {$in: _id}, ...user.organization?{organization: user.organization}:{}})
            for(let i=0; i<objects.length; i++){
            ...
            objects = await AdsZakaji.find({item: {$in: _id}}).select('image').lean()
            for (let i = 0; i < objects.length; i++) {
                await deleteFile(objects[i].image)
            }
            await AdsZakaji.updateMany({_id: {$in: _id}}, {del: 'deleted'})
            await BasketZakaji.deleteMany({item: {$in: _id}})
            await Integrate1CZakaji.deleteMany({item: {$in: _id}})
        }
        return {data: 'OK'}
    },
    restoreItem: async(parent, { _id }, {user}) => {
        if(user.role==='admin') {
            await ItemZakaji.updateMany({_id: {$in: _id}}, {del: null, status: 'active'})
        }
        return {data: 'OK'}
    }
};
