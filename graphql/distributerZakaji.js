...
const resolvers = {
    ...
    distributer: async(parent, {_id}, {user}) => {
        if((mongoose.Types.ObjectId.isValid(_id)||_id==='super')){
            return await DistributerZakaji.findOne(
                _id!=='super'?
...
const resolversMutation = {
    addDistributer: async(parent, {distributer, sales, provider}, {user}) => {
        if(['admin'].includes(user.role)){
            let _object = new DistributerZakaji({
                distributer: distributer!=='super'?distributer:null,
                sales: sales,
                provider: provider
            });
            await DistributerZakaji.create(_object)
        }
        return {data: 'OK'};
    },
    setDistributer: async(parent, {_id,  sales, provider}, {user}) => {
        let object = await DistributerZakaji.findById(_id)
        if(user.role==='admin') {
    ...
    deleteDistributer: async(parent, { _id }, {user}) => {
        if(user.role==='admin') {
            await DistributerZakaji.deleteMany({_id: {$in: _id}})
        }
        return {data: 'OK'}
    }
};
