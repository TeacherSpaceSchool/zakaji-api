...
const checkAdss = async(invoice) => {
    invoice = await InvoiceZakaji.findOne({_id: invoice})
...
const resolvers = {
    ...
    adss: async(parent, {search, organization}, {user}) => {
        if(user.role) {
            let _organization = await SubBrandZakaji.findOne({_id: organization}).select('organization').lean()
            let res = await AdsZakaji.find({
                del: {$ne: 'deleted'},
                title: {'$regex': search, '$options': 'i'},
    ...
    adsOrganizations: async(parent, ctx, {user}) => {
        if(user.role) {
            if (user.organization) {
                let distributer = await DistributerZakaji.findOne({distributer: user.organization})
                ...
                if (distributer) {
                    ...
                    distributer = await OrganizationZakaji
                ...
                let organizations = await AdsZakaji.find({del: {$ne: 'deleted'}}).distinct('organization').lean()
    ...
    ads: async(parent, ctx, {user}) => {
        if(user.role) {
            let ads = await AdsZakaji.findRandom().limit(1).lean()
            return ads[0]
        }
    }
};
...
const resolversMutation = {
    ...
    restoreAds: async(parent, { _id }, {user}) => {
        if('admin'===user.role){
            await AdsZakaji.updateMany({_id: {$in: _id}}, {del: null})
        }
        return {data: 'OK'}
    },
    deleteAds: async(parent, { _id }, {user}) => {
        if(['суперорганизация', 'организация', 'admin'].includes(user.role)){
            let objects = await AdsZakaji.find({_id: {$in: _id}}).select('image').lean()
