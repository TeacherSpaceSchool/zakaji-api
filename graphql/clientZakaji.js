...
const resolvers = {
    clientsTrashSimpleStatistic: async(parent, {search}, {user}) => {
        if(user.role==='admin'){
            let clients = await ClientZakaji
                .aggregate(
                    [
                        {
                            $match:{
        ...
        if(['менеджер', 'экспедитор', 'агент', 'суперагент', 'admin', 'суперорганизация', 'организация'].includes(user.role)) {
            ...
            if(['менеджер', 'экспедитор', 'агент', 'суперагент'].includes(user.role)) {
                ...
                if(user.onlyIntegrate){
                    clients = await Integrate1CZakaji
                ...
                accessToClient = (await OrganizationZakaji.findOne({_id: user.organization}).select('accessToClient').lean()).accessToClient
                if(!accessToClient){
    ...
    clientsSyncStatistic: async(parent, {search, organization, city}, {user}) => {
        if(user.role==='admin'){
            let clients = await ClientZakaji.find({
    ...
    client: async(parent, {_id}, {user}) => {
        ...
        if(mongoose.Types.ObjectId.isValid(_id)) {
            return await ClientZakaji.findOne({
                $or: [
                    {_id: _id},
                    {user: _id}
                ]
...
const resolversMutation = {
    addClient: async(parent, {image, name, email, city, address, phone, info, login, password, category}, {user}) => {
        if(user.role==='admin'||(user.addedClient&&['суперорганизация', 'организация', 'агент'].includes(user.role))) {
            ...
            client = new ClientZakaji(client);
            await ClientZakaji.create(client);
        }
        return {data: 'OK'}
    },
    setClient: async(parent, {_id, image, name, email, address, info, newPass, phone, login, city, device, category}, {user, res}) => {
        let object = await ClientZakaji.findOne({_id: _id})
        if(
    ...
    deleteClient: async(parent, { _id }, {user}) => {
        let objects = await ClientZakaji.find({_id: {$in: _id}})
        for(let i=0; i<objects.length; i++){
            if(user.role==='admin'){
                ...
                let districts = await DistrictZakaji.find({client: objects[i]._id})
                let index
                for(let i1=0; i1<districts.length; i1++) {
                    let agentRoutes = await AgentRouteZakaji.find({district: districts[i1]._id})
                ...
                await BasketZakaji.deleteMany({client: objects[i]._id})
                await Integrate1CZakaji.deleteMany({client: objects[i]._id})
            }
        }
        return {data: 'OK'}
    },
    clearClientsSync: async(parent, { organization }, {user}) => {
        if(user.role==='admin')
            await ClientZakaji.updateMany({sync: organization.toString()}, {$pull: { sync: organization.toString()}});
        return {data: 'OK'}
    },
