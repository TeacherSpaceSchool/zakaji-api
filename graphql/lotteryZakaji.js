const LotteryZakaji = require('../models/lotteryZakaji');
...
const resolvers = {
    lotterys: async(parent, ctx, {user}) => {
        return await LotteryZakaji.find({
    ...
    lottery: async(parent, {_id}, {user}) => {
        let res = await LotteryZakaji.findOne({
            _id: _id,
            ...user.organization?{organization: user.organization}:{}
        })
            .populate({
...
const resolversMutation = {
    addLottery: async(parent, {image, organization, text, date, prizes}, {user}) => {
        if(['admin'].includes(user.role)){
            ...
            await LotteryZakaji.create(_object)
        }
        return {data: 'OK'};
    },
    setLottery: async(parent, {_id, image, text, date, tickets, prizes, photoReports}, {user}) => {
        if(['суперорганизация', 'организация', 'admin'].includes(user.role)){
            let object = await LotteryZakaji.findById(_id)
            if (image) {
                let {stream, filename} = await image;
    ...
    deleteLottery: async(parent, { _id }, {user}) => {
        if(['суперорганизация', 'организация', 'admin'].includes(user.role)){
            let objects = await LotteryZakaji.find({_id: {$in: _id}}).select('image prizes photoReports').lean()
            ...
            await LotteryZakaji.deleteMany({_id: {$in: _id}})
