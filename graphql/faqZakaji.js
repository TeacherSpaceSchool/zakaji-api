...
const resolversMutation = {
    addFaq: async(parent, {file, title, video, typex}, {user}) => {
        if(user.role==='admin') {
            ...
            _object = await FaqZakaji.create(_object)
            return _object
        }
    },
    setFaq: async(parent, {_id, file, title, video, typex}, {user}) => {
        if(user.role==='admin') {
            let object = await FaqZakaji.findById(_id)
            if (file) {
                let {stream, filename} = await file;
    ...
    deleteFaq: async(parent, { _id }, {user}) => {
        if(user.role==='admin'){
            let objects = await FaqZakaji.find({_id: {$in: _id}}).select('file').lean()
            for(let i=0; i<objects.length; i++){
                if(objects[i].file)
                    await deleteFile(objects[i].file)
            }
            await FaqZakaji.deleteMany({_id: {$in: _id}})
        }
        return {data: 'OK'}
    }
};
