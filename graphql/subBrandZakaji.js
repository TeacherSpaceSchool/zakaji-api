...
const resolvers = {
    subBrands: async(parent, {organization, search, city}, {user}) => {
        if(['admin', 'суперагент', 'экспедитор', 'суперорганизация', 'организация', 'менеджер', 'агент', 'client'].includes(user.role)){
            ...
            return await SubBrandZakaji.find({
                del: {$ne: 'deleted'},
...
const resolversMutation = {
    addSubBrand: async(parent, {image, miniInfo, priotiry, organization, cities, name}, {user}) => {
        if('admin'===user.role){
            ...
            let _object = new SubBrandZakaji({
            ...
            _object = await SubBrandZakaji.create(_object)
            return _object
        }
        return {data: 'OK'};
    },
    setSubBrand: async(parent, {_id, image, miniInfo, priotiry, cities, name}, {user}) => {
        if('admin'===user.role){
            let object = await SubBrandZakaji.findOne({
                _id: _id
            })
            if (image) {
                let {stream, filename} = await image;
    ...
    onoffSubBrand: async(parent, { _id }, {user}) => {
        if('admin'===user.role) {
            let objects = await SubBrandZakaji.find({_id: {$in: _id}})
    ...
    deleteSubBrand: async(parent, { _id }, {user}) => {
        if('admin'===user.role){
            await SubBrandZakaji.updateMany({_id: {$in: _id}}, {del: 'deleted'})
            await ItemZakaji.updateMany({subBrand: {$in: _id}}, {subBrand: undefined})
        }
        return {data: 'OK'}
    }
};
