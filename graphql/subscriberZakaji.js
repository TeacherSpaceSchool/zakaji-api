...
const resolvers = {
    subscribers: async(parent, ctx, {user}) => {
        let res = []
        if('admin'===user.role){
            let findRes = await SubscriberZakaji
                .find({})
                .populate({ path: 'user' })
                .sort('-createdAt')
                .lean()
            for(let i=0; i<findRes.length; i++){
                ...
                if(findRes[i].user){
                    if('admin'===findRes[i].user.role) {
                        ...
                        let client = await ClientZakaji.findOne({user: findRes[i].user._id}).select('name address').lean()
                        if(client)
                        ...
                        let employment = await EmploymentZakaji.findOne({user: findRes[i].user._id}).lean()
                        if(employment)
                            res[i].user = `${findRes[i].user.role} ${employment.name}`
                    }
                    else {
                        let employment = await EmploymentZakaji.findOne({user: findRes[i].user._id}).populate({ path: 'organization' }).lean()
                        if(employment)
...
const resolversMutation = {
    deleteSubscriber: async(parent, { _id }, {user}) => {
        if('admin'===user.role){
            await SubscriberZakaji.deleteMany({_id: {$in: _id}})
        }
        return {data: 'OK'}
    }
};
