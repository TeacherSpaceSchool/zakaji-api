...
const resolvers = {
    integrate1CsSimpleStatistic: async(parent, {search, filter, organization}, {user}) => {
        if(user.role==='admin'){
            ...
            let integrate1Cs =  await Integrate1CZakaji.aggregate(
                [
                    {
                        $match:{
        ...
        if(user.role==='admin'){
            ...
            let integrate1Cs =  await Integrate1CZakaji.aggregate(
                [
                    ...
                    { $lookup:
                        {
                            from: ItemZakaji.collection.collectionName,
                            let: { item: '$item' },
                            pipeline: [
    ...
    integrate1C: async(parent, {_id}, {user}) => {
        if(mongoose.Types.ObjectId.isValid(_id)&&user.role==='admin'){
            return await Integrate1CZakaji
                .findOne({_id: _id})
                .populate({
                    path: 'agent',
                    select: '_id name'
    ...
    ecspeditorsIntegrate1C: async(parent, {organization}, {user}) => {
        if(user.role==='admin') {
            let ecspeditors =  await Integrate1CZakaji
                .find({
                    ...(organization==='super'?{organization: null}:{organization: organization}),
                    ecspeditor: {$ne: null}
                })
                .distinct('ecspeditor').lean()
            ecspeditors = await EmploymentZakaji.find({
                ...(organization==='super'?{organization: null}:{organization: organization}),
                _id: {$nin: ecspeditors},
                del: {$ne: 'deleted'}
            })
    ...
    agentsIntegrate1C: async(parent, {organization}, {user}) => {
        if(user.role==='admin') {
            let agents =  await Integrate1CZakaji
                .find({
                    ...(organization==='super'?{organization: null}:{organization: organization}),
                    agent: {$ne: null}
                })
    ...
    clientsIntegrate1C: async(parent, {organization, city}, {user}) => {
        if(user.role==='admin') {
            let clients =  await Integrate1CZakaji
                .find({
                    ...(organization==='super'?{organization: null}:{organization: organization}),
                    client: {$ne: null}
                })
            ...
            if(organization!=='super')
                organization = await OrganizationZakaji.findOne({_id: organization}).lean()
            if(organization.accessToClient||organization==='super') {
                ...
                let clients1 = await OrderZakaji.find({item: {$in: items}}).distinct('client').lean()
                clients = await ClientZakaji.find({
                    $and: [
                        {_id: {$in: clients1}},
                        {_id: { $nin: clients}}
    ...
    itemsIntegrate1C: async(parent, {organization, city}, {user}) => {
        if(mongoose.Types.ObjectId.isValid(organization)&&user.role==='admin') {
            let items =  await Integrate1CZakaji
                .find({
                    organization: organization,
                    item: {$ne: null}
                })
                .distinct('item').lean()
            items = await ItemZakaji.find({
                _id: {$nin: items},
                ...city?{city: city}:{},
                organization: organization,
                del: {$ne: 'deleted'}
...
const resolversMutation = {
    ...
    setIntegrate1C: async(parent, {_id, item, client, guid, agent, ecspeditor}, {user}) => {
        if(user.role==='admin') {
            let object = await Integrate1CZakaji.findById(_id)
            ...
            await object.save();
            let res = await Integrate1CZakaji
    ...
    deleteIntegrate1C: async(parent, { _id }, {user}) => {
        if(user.role==='admin')
            await Integrate1CZakaji.deleteMany({_id: {$in: _id}})
        return {data: 'OK'}
    },
        ...
        if(user.role==='admin'){
            ...
            for(let i = 0;i<rows.length;i++){
                if(mongoose.Types.ObjectId.isValid(rows[i][0])&&rows[i][1]&&rows[i][1].length>0) {
                    let client = await ClientZakaji.findOne({_id: rows[i][0]}).select('_id').lean()
                    if (client) {
                        ...
                        if(integrate1CZakaji) {
                            integrate1CZakaji.guid = rows[i][1]
                            await integrate1CZakaji.save()
                        }
                        else {
                            let _object = new Integrate1CZakaji({
                                item: null,
