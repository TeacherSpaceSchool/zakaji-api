...
const resolvers = {
    unloadingInvoicesFromRouting: async(parent, { orders, organization }, {user}) => {
        if(['admin', 'агент', 'суперорганизация', 'организация', 'менеджер', 'экспедитор', 'суперэкспедитор'].includes(user.role)){
            if(organization!=='super') {
                organization = await OrganizationZakaji.findOne({_id: organization}).select('name address phone').lean()
            }
            else {
                organization = await ContactZakaji.findOne().select('name address phone').lean()
            }
            ...
            for(let i = 0; i<data.length;i++){
                ...
                consignation = 0
                consigmentOrder = await InvoiceZakaji.find({consignmentPrice: {$gt: 0}, paymentConsignation: {$ne: true}, client: data[i].client._id}).select('consignmentPrice').lean()
    ...
    listDownload: async(parent, {orders}, {user}) => {
        if(['admin', 'агент', 'суперорганизация', 'организация', 'суперэкспедитор', 'экспедитор', 'менеджер'].includes(user.role)){
            orders = await InvoiceZakaji.find({_id: {$in: orders}})
    ...
    routes: async(parent, {organization, search, sort, filter, date, skip}, {user}) => {
        if(['суперорганизация', 'организация', 'менеджер', 'агент', 'admin', 'экспедитор', 'суперэкспедитор'].includes(user.role)) {
            ...
            let district
            if(search.length>0){
                _employments = await EmploymentZakaji.find({
                    name: {'$regex': search, '$options': 'i'}
                }).distinct('_id').lean()
            }
            if(['менеджер', 'агент'].includes(user.role)) {
                district = await DistrictZakaji.find({
                    agent: user.employment
                }).distinct('_id').lean()
            }
            return await RouteZakaji.find({
    ...
    route: async(parent, {_id}, {user}) => {
        if(mongoose.Types.ObjectId.isValid(_id)) {
            let route = await RouteZakaji.findOne({_id: _id})
                .populate({
                    path: 'selectEcspeditor',
                    select: 'name _id'
                })
...
const resolversMutation = {
    buildRoute: async(parent, {provider, autoTonnage, orders, length}, {user}) => {
        if(['admin', 'агент', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)){
            if(provider!=='super')
                provider = (await OrganizationZakaji.findOne({_id: provider}).select('warehouse').lean()).warehouse.replace(', ', ',');
            else
                provider = (await ContactZakaji.findOne().select('warehouse').lean()).warehouse.replace(', ', ',');
            if(provider&&provider.length>0){
                let invoices = await InvoiceZakaji.find({_id: {$in: orders}}).lean();
    ...
    setRoute: async(parent, {route, deletedOrders}, {user}) => {
        if(['admin', 'агент', 'суперорганизация', 'организация', 'менеджер', 'экспедитор', 'суперэкспедитор'].includes(user.role)){
            route = await RouteZakaji.findOne({_id: route, ...user.organization?{provider: user.organization}:{}})
            if(route){
                ...
                await InvoiceZakaji.updateMany({_id: {$in: deletedOrders}}, {distributed: false})
                await route.save()
            }
        }
        return {data: 'OK'};
    ...
    addRoute: async(parent, {deliverys, provider, selectProdusers, selectDistricts, selectEcspeditor, selectAuto, selectedOrders, dateDelivery, allTonnage}, {user}) => {
        if(['admin', 'агент', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)){
            ...
            while (await RouteZakaji.findOne({number: number}).select('_id').lean())
                number = await randomstring.generate({length: 12, charset: 'numeric'});
            let _object = new RouteZakaji({
                deliverys: deliverys,
            ...
            await RouteZakaji.create(_object);
            await InvoiceZakaji.updateMany({_id: {$in: selectedOrders}}, {distributed: true})
        }
        return {data: 'OK'};
    },
    deleteRoute: async(parent, { _id, selectedOrders }, {user}) => {
        if(!['экспедитор', 'суперэкспедитор'].includes(user.role)){
            await RouteZakaji.deleteMany({_id: _id})
            await InvoiceZakaji.updateMany({_id: {$in: selectedOrders}}, {distributed: false})
        }
        return {data: 'OK'}
    }
};
