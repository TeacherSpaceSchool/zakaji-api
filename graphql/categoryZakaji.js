...
const resolvers = {
    categorys: async(parent, {search, sort, filter}, {user}) => {
        return [
            ...user.role==='admin'?[await CategoryZakaji.findOne({name: 'Не задано'}).lean()]:[],
            ...(await CategoryZakaji.find({
                    $and: [
                        {name: {$ne: 'Не задано'}},
    ...
    category: async(parent, {_id}) => {
        if(_id!=='all')
            return await CategoryZakaji.findOne({
                _id: _id
            })
                .lean()
        else return null
...
const resolversMutation = {
    addCategory: async(parent, {image, name}, {user}) => {
        if(user.role==='admin'&&name!=='Не задано'){
            ...
            _object = await CategoryZakaji.create(_object)
            return _object
        }
    },
    setCategory: async(parent, {_id, image, name}, {user}) => {
        if(user.role==='admin'&&name!=='Не задано') {
            let object = await CategoryZakaji.findById(_id)
            if (image) {
                let {stream, filename} = await image;
    ...
    deleteCategory: async(parent, { _id }, {user}) => {
        if(user.role==='admin'){
            let objects = await CategoryZakaji.find({_id: {$in: _id}, name: {$ne: 'Не задано'}}).select('image').lean()
            for(let i=0; i<objects.length; i++){
                await deleteFile(objects[i].image)
            }

            let categoryUndefined = await CategoryZakaji.findOne({name: 'Не задано'}).select('_id').lean()
            await SubCategoryZakaji.updateMany({category: {$in: _id}}, {category: categoryUndefined._id})

            await CategoryZakaji.deleteMany({_id: {$in: _id}, name: {$ne: 'Не задано'}})
        }
        return {data: 'OK'}
    },
    onoffCategory: async(parent, { _id }, {user}) => {
        if(user.role==='admin'){
            let objects = await CategoryZakaji.find({_id: {$in: _id}})
            for(let i=0; i<objects.length; i++){
