...
module.exports.reductionToEmployment = async() => {
    ...
    for(let i=0; i<createdEmployment.length; i++) {
        employment = await EmploymentZakaji.findOne({email: createdEmployment[i].email}).lean()
        if(!employment) {
            employment = new EmploymentZakaji({...createdEmployment[i], del: 'deleted'});
            employment = await EmploymentZakaji.create(employment);
            count = await InvoiceZakaji.countDocuments({agent: new mongoose.Types.ObjectId(employment.email)})
            await InvoiceZakaji.updateMany({agent: new mongoose.Types.ObjectId(employment.email)}, {agent: employment._id})
    ...
    for(let i=0; i<deletedEmployment.length; i++) {
        employment = await EmploymentZakaji.findOne({_id: deletedEmployment[i]}).lean()
        if(employment)
