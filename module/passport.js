...
//настройка паспорта
    passport.use(new LocalStrategy({
        ...
        function (login, password, done) {
            UserZakaji.findOne({login: login}, (err, user) => {
                if (err) {
                    return done(err);
                }
    ...
    passport.use(new JwtStrategy(jwtOptions, function (payload, done) {
        UserZakaji.findOne({login:payload.login}, (err, user) => {
            if (err) {
                return done(err)
            }
            if (user) {
        ...
        try{
            if (user&&user.status==='active') {
                if('admin'===user.role)
                    ...
                    let client = await ClientZakaji.findOne({user: user._id}).select('client category city')
                    ...
                    let employment = await EmploymentZakaji.findOne({user: user._id}).select('_id').lean()
                    user.employment = employment._id
                    resolve(user)
                }
                else {
                    let employment = await EmploymentZakaji.findOne({user: user._id})
...
const signupuser = async (req, res) => {
    try{
        let _user = new UserZakaji({
            login: req.query.login,
            role: 'client',
            status: 'active',
            password: req.query.password,
        });
        const user = await UserZakaji.create(_user);
        const payload = {
            id: user._id,
            login: user.login,
            status: user.status,
...
const signupuserGQL = async ({password, login}, res) => {
    try{
        //await UserZakaji.deleteMany()
        let user = new UserZakaji({
            login: login.trim(),
            role: 'client',
            status: 'active',
            password: password,
        });
        user = await UserZakaji.create(user);
        const client = new ClientZakaji({
            name: '',
            email: '',
            address: [],
            info: '',
            ...
            try{
                if (user&&user.status==='active') {
                    ...
                    if(!['admin', 'client'].includes(user.role)) {
                        let employment = await EmploymentZakaji.findOne({user: user._id}).select('organization').lean()
